...
const ReservaManagement: React.FC<ReservaManagementProps> = ({ cancha, onBack }) => {
  ...
  const getEstadoReserva = (inicia_en: string, termina_en: string) => {
    const ahora = new Date();
    const inicio = new Date(inicia_en);
    const fin = new Date(termina_en);
      ...
      {reservas.length === 0 ? (
        ...
        <div className="space-y-4">
          {reservas.map((reserva) => {
            const inicioFormat = formatDateTime(reserva.inicia_en);
            const finFormat = formatDateTime(reserva.termina_en);
            const duracion = calcularDuracion(reserva.inicia_en, reserva.termina_en);
            const estadoInfo = getEstadoReserva(reserva.inicia_en, reserva.termina_en);
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900">
                      {selectedReserva.usuario?.persona 
                        ...
                        : `Cliente ID: ${selectedReserva.id_cliente}`
                      }
                    </p>
                  </div>
                  {selectedReserva.usuario?.persona && (
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-900">
                      {formatDateTime(selectedReserva.inicia_en).date} a las {formatDateTime(selectedReserva.inicia_en).time}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500">Fecha y hora de fin</label>
                    <p className="text-sm text-gray-900">
                      {formatDateTime(selectedReserva.termina_en).date} a las {formatDateTime(selectedReserva.termina_en).time}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500">Duración</label>
                    <p className="text-sm text-gray-900">
                      {calcularDuracion(selectedReserva.inicia_en, selectedReserva.termina_en)}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500">Cantidad de personas</label>
                    <p className="text-sm text-gray-900">{selectedReserva.cantidad_personas} personas</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-900">Bs. {selectedReserva.monto_base}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500">Monto extra</label>
                    <p className="text-sm text-gray-900">Bs. {selectedReserva.monto_extra}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500">Monto total</label>
                    <p className="text-lg font-semibold text-green-600">Bs. {selectedReserva.monto_total}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-yellow-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-900">
                      {selectedReserva.requiere_aprobacion ? 'Sí' : 'No'}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500">Estado</label>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getEstadoReserva(selectedReserva.inicia_en, selectedReserva.termina_en).color}`}>
                      {getEstadoReserva(selectedReserva.inicia_en, selectedReserva.termina_en).estado}
                    </span>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500">Creado el</label>
                    <p className="text-sm text-gray-900">
                      {formatDateTime(selectedReserva.creado_en).date} a las {formatDateTime(selectedReserva.creado_en).time}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500">Última actualización</label>
                    <p className="text-sm text-gray-900">
                      {formatDateTime(selectedReserva.actualizado_en).date} a las {formatDateTime(selectedReserva.actualizado_en).time}
                    </p>
                  </div>
                </div>
              </div>
