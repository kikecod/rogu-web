...
import ReservaManagement from '../../reservas/components/ReservaManagement';
...
const CanchaManagement: React.FC<CanchaManagementProps> = ({ sede, onBack }) => {
  ...
  const loadCanchas = async () => {
    try {
      ...
      for (const cancha of sedeCanchas) {
        try {
          const partes = await canchaService.loadPartes(cancha.id_cancha);
          cancha.parte = partes;
        } catch (error) {
          console.error(`Error loading partes for cancha ${cancha.id_cancha}:`, error);
          cancha.parte = [];
        }
      }
  ...
  const handleDelete = async (id_cancha: number) => {
    if (!confirm('¿Estás seguro de que quieres eliminar esta cancha?')) return;

    try {
      await canchaService.delete(id_cancha);
      await loadCanchas();
    } catch (error) {
  ...
  const openDisciplinaModal = (cancha: Cancha) => {
    ...
    const canchaParteDisciplinas = cancha.parte?.map(p => p.id_disciplina).filter(id => id) || [];
  ...
  const saveDisciplinas = async () => {
    ...
    try {
      await canchaService.updateDisciplinas(selectedCancha.id_cancha, selectedDisciplinas);
      await loadCanchas();
    ...
    <div className="space-y-6">
      ...
      {canchas.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {canchas.map((cancha) => (
            <div key={cancha.id_cancha} className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {cancha.parte.map((parte: any, index: number) => (
                        ...
                        >
                          {parte.disciplina?.nombre || `Disciplina ${parte.id_disciplina}`}
                        </span>
                      ))}
                    </div>
                  </div>
          ...
          <div className="relative top-20 mx-auto p-5 border w-11/12 max-w-2xl shadow-lg rounded-md bg-white">
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {disciplinas.map((disciplina) => (
                <div key={disciplina.id_disciplina} className="flex items-center space-x-3 p-3 border rounded-md">
                  <input
                    type="checkbox"
                    checked={selectedDisciplinas.includes(disciplina.id_disciplina)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setSelectedDisciplinas([...selectedDisciplinas, disciplina.id_disciplina]);
                      } else {
                        setSelectedDisciplinas(selectedDisciplinas.filter(id => id !== disciplina.id_disciplina));
                      }
                    }}
                    className="mr-2"
                  />
