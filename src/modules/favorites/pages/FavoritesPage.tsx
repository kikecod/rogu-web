...
import FavoriteFilters, { type FavoriteFilterState } from '../components/FavoriteFilters';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-white">
      <div className="mx-auto max-w-7xl px-4 pt-10">
        {/* Layout principal: contenido + aside sticky */}
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr,360px]">
          {/* Columna izquierda: header + grid */}
          <section>
            {/* Header compacto */}
            <div className="mb-4 flex flex-col justify-between gap-4 sm:mb-6 md:flex-row md:items-end">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-600">
                  {favorites.length} {favorites.length === 1 ? 'cancha' : 'canchas'} guardadas
                </p>
              </div>
            ...
            <div className="pb-10">
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
                {favorites.map((fav) => (
                  <FavoriteCard
                    key={`${fav.idCliente}-${fav.idCancha}-${fav.creadoEn}`}
                    favorite={fav}
                    onRemove={removeFavorite}
                    onReserve={(idCancha) => navigate(`/field/${idCancha}`)}
                  />
                ))}
              </div>
            </div>
