...
import { useDistrictAutocomplete } from '../hooks';
...
const DistrictAutocomplete: React.FC<DistrictAutocompleteProps> = ({
  ...
  // Limpiar cuando cambia la ciudad
  useEffect(() => {
    if (inputValue) {
      setInputValue('');
      clearSuggestions();
  ...
  }, [city, clearSuggestions, inputValue]);
  ...
  // Manejar focus
  const handleFocus = () => {
    if (canSearch && inputValue.length >= 2) {
      setShowDropdown(true);
    }
  };
    ...
    <div className={`relative ${className}`} ref={dropdownRef}>
      <div className="relative">
        <input
          ...
          placeholder={canSearch ? placeholder : 'Selecciona una ciudad primero'}
          disabled={isDisabled}
          className={`
            ...
            ${!canSearch ? 'bg-gray-50 text-gray-500' : ''}
          `}
        />
        ...
        <Map className={`
          ...
          ${!canSearch ? 'text-gray-300' : 'text-gray-400'}
        `} />
      ...
      {/* Mensaje de ayuda cuando no hay ciudad */}
      {!canSearch && (
        <div className="mt-1 text-xs text-gray-500">
          Primero debes seleccionar una ciudad para buscar distritos
        </div>
      )}

      {/* Dropdown de sugerencias */}
      {showDropdown && canSearch && (hasSuggestions || loading || error) && (
          ...
          {hasSuggestions && (
            <>
                ...
                >
                  <div className="flex justify-between items-center">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="text-gray-900">{suggestion.value}</div>
              ...
              {suggestions.length === 0 && inputValue.length >= 2 && !loading && (
                <div className="p-4 text-center text-sm text-gray-500">
                  No se encontraron distritos con "{inputValue}" en {city}
                </div>
              )}
            </>
          )}
